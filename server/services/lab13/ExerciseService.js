...
async function getExercise(data) {
  try {
    const exerciseResponse = await db.ExerciseLab13.findOne(
      {
...
async function postExercise(data) {
  try {
    ...
    if (!getExerciseResponse) {
      // adds in new entry
      return await db.ExerciseLab13.create(newExercise).id;
    } else {
      ...
      await db.ExerciseLab13.create(updatedExercise).id;
