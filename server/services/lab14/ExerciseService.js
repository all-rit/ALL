...
async function getExercise(data) {
  try {
    const exerciseResponse = await db.ExerciseLab14.findOne({
...
async function postExercise(data) {
  try {
    const { userId, isExerciseComplete, hasViewed } = data;
    ...
    if (!getExerciseResponse) {
      // adds in new entry
      return await db.ExerciseLab14.create(newExercise).id;
    } else {
      ...
      await db.ExerciseLab14.create(updatedExercise).id;
