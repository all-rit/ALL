...
const createNewBoard = () => {
    ...
    return {board : [[null,null,null] , [null,null, null] ,[null,null, null]], available : openSpots}; 
};
/**
...
const checkWinner = (GameInfo) => {
    ...
    Players.forEach((Player) => {
        for(let y = 0; y < 3 ; y++){
            if(check3InARow(GameState.board[y][0],
            GameState.board[y][1],
            GameState.board[y][2]) &&
        ...
        // check vertical
        for(let x = 0; x < 3; x++){
            if (check3InARow(GameState.board[0][x],GameState.board[1][x], GameState.board[2][x]) && Player.piece === GameState.board[0][x]){
                winner = Player;
            }
        }
            // check diagonal formations
           if((check3InARow(GameState?.board[0][0], GameState?.board[1][1], GameState?.board[2][2])||check3InARow(GameState?.board[2][0], GameState?.board[1][1], GameState?.board[0][2]))&&(Player.piece=== GameState.board[1][1])){
                winner = Player; 
            }
    });
    if(winner !== null){
...
const check3InARow = (first, second, third) => {
    return (((first === second) && (first === third)) && (second === third) && ((first !== null)&&(second !== null)&&(third!==null)));
};
...
export default {makeMove, createNewBoard, Players, checkWinner ,  GameState, aIMove};
