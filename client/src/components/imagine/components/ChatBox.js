...
const ChatBox =()=>{
    ...
    useEffect(()=>{
        if(messages?.length===0){
            const randomAMessage = fakeUserAMessages[Math.floor(Math.random() * fakeUserAMessages.length)];
            const randomBMessage = fakeUserBMessages[Math.floor(Math.random() * fakeUserBMessages.length)];
            let messArr=["PikachuRising: "+randomAMessage,"Your Mama: "+randomBMessage]
            setMessages(messArr)
        }
            ...
            <div>
                <Form className="tw-bg-white">
                    <FormGroup className="tw-bg-white">
                        <CustomInput className="tw-bg-white" type="select" id="Lobby"  multiple disabled>
                ...
                <Form inline onSubmit={handleSubmit}>
                    <FormGroup className="tw-max-w-xs tw-w-full ">
                        <CustomInput placeholder="Enter Chat..." className="tw-w-full tw-h-9" type="text" id="userMessage" name="userMessage" />
                    </FormGroup>
