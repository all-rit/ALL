...
const AddModal = (props) => {
    ...
    const handleInviteCodeSubmit = e => {
        ...
        if (!inviteCode || inviteCode.indexOf(' ') >= 0){  //check if the code is empty or has white space in between
            alert("Invite code cannot be empty or have spaces.")
        } else {
            GroupService.enrollUser(user.userid, inviteCode.toUpperCase()).then((response) => {
                    ...
                    <Modal isOpen={modal} toggle={toggle} className="add_instr_grp_modal">
                        ...
                        <Form onSubmit={handleInviteCodeSubmit}>
                            <ModalBody>
                                    ...
                                    <Input
                                        type="text" name="inviteCode" id="inviteCode" onChange={(e => {
                                            setInviteCode(e.target.value)
                                        })}
                                    />
                            </ModalBody>
                            <ModalFooter>
                                <Button color="primary" onClick={handleInviteCodeSubmit}>Enroll in Group</Button>
