...
const RepairService = {
    ...
    submitRepairSkip: (skiptomain) => {
        return API.postWithBody(process.env.REACT_APP_SERVER_URL + endpoints.SUBMIT_REPAIR_SKIP, {
            skiptomain
        });
    },
    submitRepairHint: (tabindex) => {
        return API.postWithBody(process.env.REACT_APP_SERVER_URL + endpoints.SUBMIT_REPAIR_HINT, {
            tabindex
        });
    },
};
