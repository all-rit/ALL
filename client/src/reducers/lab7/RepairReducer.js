...
const RepairReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case types.UPDATE_MAKE_DECISION:
            return {
                ...state,
                updateMakeDecision: action.func
            }
        default:
            return state;
    }
...
export const actions = {
    updateRepairEquation: (rewardValue, costValue) => ({
        ...
        rewardValue, costValue,
    }),
    updateRewardError: (rewardError) => ({type: types.UPDATE_REWARD_ERROR, rewardError}),
    updateCostError: (costError) => ({type: types.UPDATE_COST_ERROR, costError}),
